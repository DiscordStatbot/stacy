  ...
  run: async (bot, message, args, settings) => {
    ...
    try {
      ...
      if (args[0] === 'list') {
        ...
        const hdChan = message.guild.channels.get(args[1]) || message.mentions.channels.first();
        ...
        const check = message.content.split(' | ').slice(1);
        const question = check[0];
        const response = check[1];
        ...
        switch (updated) {
          ...
          case 'support': {
            if (!helpDesk) return message.channel.send(`There is no help desk setup for this channel. Please setup the help desk with \`${settings.prefix}desk <#channel or ID> create\``);
            const sChan = message.mentions.channels.last() || message.guild.roles.get(args[2]);
            if (sChan) {
              try {
          ...
          case 'setup': {
            ...
            const defColor = bot.config.yellow;
          ...
          case 'remove': {
            ...
            try {
              await bot.updateDesk(hdChan, { $pull: { qar: { question } } }, { multi: true });
              await bot.updateDesk(hdChan, { $inc: { deskNum: -1 } });
              const aembed = new RichEmbed()
                .setColor(bot.config.green)
