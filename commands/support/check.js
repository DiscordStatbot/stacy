...
module.exports = {
  ...
  run: async (bot, message, args, settings) => {
    try{
      ...
      switch(option) {
      case 'user': {
        let member = message.mentions.members.first() || message.guild.members.find(args[1]);
        let createdAt = dateFormat(member.user.createdAt, 'isoDateTime');

        let list = member.roles.map(r => ` \`${r.name} \``).slice(1).join(' | ');
      ...
      case 'role': {
        let role = message.mentions.roles.first() || message.guild.roles.find(args[1]);
        let embed = new RichEmbed()
      ...
      case 'server': {
        let roles = message.guild.roles.map(r => ` \`${r.name} \``).slice(1).join(' | ');
        let textChannels = message.guild.channels.filter(c => c.type == 'text').map(c => ` \`${c.name}\``).join(' | ');
