...
module.exports = {
  ...
  run: async (bot, message, args, settings) => {
    try{
      ...
      let closer = message.author;
      let userID = message.channel.name.slice(7);
      let test = 'none';
  
      let ticketed = message.guild.members.get(userID);

      let logChan = message.guild.channels.get(settings.ticketLog);

      message.channel.send('Ticket is being closed!').then(async () => {
        await message.channel.fetchMessages().then(async messages => {
          let content = messages.filter(u => u.author != bot.user && !u.content.startsWith(settings.prefix)).map(m => `[ ${dateFormat(m.createdTimestamp, 'isoDateTime')} ] ${m.author.username} (${m.author.id}) - ${m.content}`).join('\n');
          ...
          fs.writeFile(
            ...
            async function (error) {
              ...
              let attachment = new Attachment(`./logs/${message.guild.id}_${userID}_${message.createdTimestamp}.txt`, `${message.guild.id}_${userID}_${message.createdTimestamp}.txt`);
              await logChan.send(`Thread closed for ${ticketed.user.username}`, attachment);
            });

        }).then(async () =>{
          await message.channel.delete().then(async () => {
            fs.unlink(`./logs/${message.guild.id}_${userID}_${message.createdTimestamp}.txt`, function (error) {
              if (error) return console.error(` [ERROR] ${error.stack}`);
            });
          }); 
        });
