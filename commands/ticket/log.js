    ...
    try{
      ...
      fs.writeFile(
        ...
        async function (error) {
          ...
          let attachment = new Attachment(`./logs/${message.guild.id}_${member.id}_${message.id}.txt`, `${message.guild.id}_${member.id}_${message.id}.txt`);
          await message.channel.send(`Ticket logs for **${member.user.username}**\n\n**Total Logs:** ${userTickets.length}`, attachment).then(async () => {
            fs.unlink(`./logs/${message.guild.id}_${member.id}_${message.id}.txt`, function (error) {
              if (error) return console.error(` [ERROR] ${error.stack}`);
            });
          });
        });
