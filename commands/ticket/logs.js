    ...
    try{
      ...
      fs.writeFile(
        ...
        async function (error) {
          ...
          let attachment = new Attachment(`./logs/${message.guild.id}_${message.id}.txt`, `${message.guild.id}_${message.id}.txt`);
          await message.channel.send(`Ticket logs for **${message.guild.name}**\n\n**Total Logs:** ${guildTickets.length}`, attachment).then(async () => {
            fs.unlink(`./logs/${message.guild.id}_${message.id}.txt`, function (error) {
              if (error) return console.error(` [ERROR] ${error.stack}`);
            });
          });
        });
